...
wget https://raw.githubusercontent.com/HiroPH/Virtu-Custom/main/udp/config.json -O /etc/zivpn/config.json 1> /dev/null 2> /dev/null

echo "Generating cert files:"
openssl req -new -newkey rsa:4096 -days 365 -nodes -x509 -subj "/C=US/ST=California/L=Los Angeles/O=Example Corp/OU=IT Department/CN=HirokiPH" -keyout "/etc/zivpn/zivpn.key" -out "/etc/zivpn/zivpn.crt"
